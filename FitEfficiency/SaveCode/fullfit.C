...
void fit_trigger_eta(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
 ...
 for(int ii=0;ii<num_data.size();ii++)
  {
   if(ZYbin[turn_1[ii]]<2 && ZYbin[turn_1[ii]]>40) continue;
   if(ZYbin[turn_2[ii]]<2 && ZYbin[turn_2[ii]]>40) continue;
...
//   if(err_mc[turn_2[ii]]/num_mc[turn_2[ii]] > 0.05) continue;
   ...
   if(fabs(index_trigger_1[turn_1[ii]]-index_trigger_2[turn_1[ii]]) >= 14 ) continue; 
...
//   outfile<<"  chi2 for constraint: "<<ii<<"  "<<chi2_cal<<"  "<<ratio_1<<"  "<<err_1<<"  "<<ratio_2<<"  "<<err_2<<"  "<<obs_data<<"  "<<obs_err<<endl;
 ...
 outfile<<" *** total chi2: "<<chi2_sum<<endl;
 f = chi2_sum;
 outfile.close();
...
//         if(ibin!=19) myMinuit->mnparm(ibin, name, 0.8, 0.1, 0, 5, ierflg);
          myMinuit->mnparm(ibin, name, MyR->Gaus(1, 0.0001)*tmptrigger[ibin], 0.1, 0, 5, ierflg);
//         if(ibin!=19) myMinuit->mnparm(ibin, name, tmptrigger[ibin], 0.1, 0, 5, ierflg);
/*
         if(ibin==19)
          {
           myMinuit->mnparm(ibin, name, tmptrigger[ibin], 0.1, 0, 5, ierflg);
...
//         if(ibin!=19) myMinuit->mnparm(ibin+44, name, 1.0, 0.1, 0.0, 1.5, ierflg);
         myMinuit->mnparm(ibin+44, name, MyR->Gaus(1, 0.0001)*tmpselection[ibin], 0.1, 0.0, 1.5, ierflg);
//         if(ibin!=19) myMinuit->mnparm(ibin+44, name, tmpselection[ibin], 0.1, 0.0, 1.5, ierflg);
/*
         if(ibin==19) 
          {
           myMinuit->mnparm(ibin+44, name, tmpselection[ibin], 0.1, 0.0, 1.5, ierflg);
