...
//    if(ix==20) tmpselection[ix] = tmpselection[ix]*(0.99 + 0.02*MyR->Rndm());
  ...
  for(int iloop=0;iloop<1000000;iloop++)
   {//try starting point
    if(singleloop>5000 && isInSingleLoop)
     {
    ...
    if(chi2_sum<min_chi2)
     {
      ...
      double diff;
      diff = (1-10*(1-chi2_sum/min_chi2));
      if(diff<0) diff = 0.01;
      step_trigger = step_trigger * diff;
      step_selection = step_selection * diff;

      if(step_trigger<0.001) step_trigger = 0.001;
      if(step_selection<0.002) step_selection = 0.002;
       ...
       for(int ibin=0;ibin<44;ibin++)
        {
         ...
         myMinuit->mnparm(ibin, name, iter_trigger[ibin], 0.01, 0, 1, ierflg);
        }
       for(int ibin=0;ibin<44;ibin++)
        {
         sprintf(name, "%s%d", "scale_cut_",ibin);
         myMinuit->mnparm(ibin+44, name, iter_selection[ibin], 0.01, 0.0, 1.5, ierflg);
        }
