...
{      
 ...
 countings = 0;
  infile.open("Inputfiles/trigger_selection_mc_trigger.txt",ios::in);
 while(infile>>skip_ZY)
  {
   start_count = 0;
   while(infile>>skip)
    {
     ...
     if(skip==999111)
      {
       infile>>skip;
       infile>>index_reference;//reference:bin with the largest statistic in the specific ZY bin
       infile>>eta1_reference;
       infile>>eta2_reference;
       break;
      }
...
//   outfile_rndm.open("random_starting.txt",ios::out);
  ...
  double lowedge_trigger = 0.975;
  double step_trigger = 0.05;
...
//    if(ix==20) tmpselection[ix] = tmpselection[ix]*(0.99 + 0.02*MyR->Rndm());
  ...
  //************************************//
  //start looping to find global minimum//
  //************************************//

  for(int iloop=0;iloop<250000;iloop++)//global loop
   {//try starting point
    if(singleloop>800)
     {
      for(int ix=0;ix<14;ix++)
