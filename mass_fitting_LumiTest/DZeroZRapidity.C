...
int main(int argc, char **argv)
{
 for(int ilumi=1;ilumi<=3;ilumi++)
 {
    ...
    fstream fit_result("fit_"+(TString)lumix+"_result.txt",ios::out);
    TFile *DataFile = new TFile("../muon_eff_LumiTest/result/data_"+(TString)lumix+"_result.root");
    TFile *BKGFile = new TFile("../muon_eff_LumiTest/EW_norm_"+(TString)lumix+".root");
    TFile *MCFile = new TFile("../muon_eff_LumiTest/result/MC_0_"+(TString)lumix+"_result.root");

    TFile *WriteFile = new TFile("EfficiencyResult_"+(TString)lumix+".root", "RECREATE");
    ...
    // For Eta Bin
    {
	    ...
	    int BinNumber;
            ...
            //draw plots!!!
            sprintf(name, "./figure_"+(TString)lumix+"/Mass_ProbeEtaBin%d_LeptonPtBin%d.eps", i, j);
