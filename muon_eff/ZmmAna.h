...
class ZmmAna
{
 public:
  ...
  //function
  ZmmAna(const char *FileList, const char *TypeName,const char *BagNo,string str);
  virtual ~ZmmAna();
...
inline ZmmAna::ZmmAna(const char *FileList,const char *TypeName,const char *BagNo,string str)
{//func_ZmmAna
 ...
 myHist = new MakeHist();
 TString histName;
 histName = "result/" + (TString)TypeName + "_" + (TString)BagNo + "_result.root";  
 if(isEWMC==true)
 {
  histName = "result_EW/" + (TString)BagNo + "_result.root";
 }
 if(Is_SS == true)
  histName = "./QCD_num/" + (TString)TypeName + "_" +(TString)BagNo + "_result.root";
