...
{//func_Loop
 ...
 for(int ii=0;ii<Nentries;ii++)
 {
  ...
  if(test=="lumi1")
   if(!(zdata_instlum>=0 && zdata_instlum<=3))continue;
  if(test=="lumi2")
   if(!(zdata_instlum>=3 && zdata_instlum<=6))continue;
  if(test=="lumi3")
   if(!(zdata_instlum>=6 && zdata_instlum<=10))continue;
...
/*   if(zdata_muon_pT[itag]<20)
   ...
   {//isTag
    ...
    {//Probe muon
     ...
     //supplement:for selection of analysis
     if(zdata_muon_mark[iprobe]!=1)isProbe=0;
     if(zdata_muon_mtc_nseg[iprobe]<1)isProbe=0;
     if(zdata_muon_trk_chi2[iprobe]>4)isProbe=0;
     if(zdata_muon_trk_nsmt[iprobe]>0)
     {
      if(fabs(zdata_muon_trk_dca[iprobe])>0.012)isProbe=0;
     }
     if(zdata_muon_trk_nsmt[iprobe]==0)
     {
      if(fabs(zdata_muon_trk_dca[iprobe])>0.2)isProbe=0;
     }
     //supplement end
     ...
     Zboson = muon_tag + muon_probe;
     //if(Zboson.M()<75 || Zboson.M()>107)isProbe=0; 
     if(Zboson.M()<74 || Zboson.M()>110)isProbe=0;
     ...
     if(isProbe==1)
     {
      ...
      if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_LeptonEta_angleL_deno->Fill(muon_probe.Eta(),total_weight);
      if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_LeptonEta_angleS_deno->Fill(muon_probe.Eta(),total_weight);
      if(zdata_instlum>=0 && zdata_instlum<=3)
      {
       if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_Zrapidity_lumi1_angleL_deno->Fill(Zboson.Rapidity(),total_weight);
       if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_Zrapidity_lumi1_angleS_deno->Fill(Zboson.Rapidity(),total_weight); 
      }
      if(zdata_instlum>=3 && zdata_instlum<=6)
      { 
       if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_Zrapidity_lumi2_angleL_deno->Fill(Zboson.Rapidity(),total_weight);
       if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_Zrapidity_lumi2_angleS_deno->Fill(Zboson.Rapidity(),total_weight);
      }
      if(zdata_instlum>=6 && zdata_instlum<=10)
      { 
       if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_Zrapidity_lumi3_angleL_deno->Fill(Zboson.Rapidity(),total_weight);
       if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_Zrapidity_lumi3_angleS_deno->Fill(Zboson.Rapidity(),total_weight);
      }
      ...
      &&((zdata_muon_etHalo[iprobe]-0.005*zdata_instlum)/muon_probe.Pt())<0.4)//cal isolation
      {
       ...
       if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_LeptonEta_angleL_nume->Fill(muon_probe.Eta(),total_weight);
       if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_LeptonEta_angleS_nume->Fill(muon_probe.Eta(),total_weight);
       if(zdata_instlum>=0 && zdata_instlum<=3)
      { 
       if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_Zrapidity_lumi1_angleL_nume->Fill(Zboson.Rapidity(),total_weight);
       if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_Zrapidity_lumi1_angleS_nume->Fill(Zboson.Rapidity(),total_weight);
      }
      if(zdata_instlum>=3 && zdata_instlum<=6)
      {
       if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_Zrapidity_lumi2_angleL_nume->Fill(Zboson.Rapidity(),total_weight);
       if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_Zrapidity_lumi2_angleS_nume->Fill(Zboson.Rapidity(),total_weight);
      }
      if(zdata_instlum>=6 && zdata_instlum<=10)
      {
       if(fabs(muon_tag.Eta()-muon_probe.Eta())>0.8)myHist->Iso_Zrapidity_lumi3_angleL_nume->Fill(Zboson.Rapidity(),total_weight);
       if(fabs(muon_tag.Eta()-muon_probe.Eta())<0.8)myHist->Iso_Zrapidity_lumi3_angleS_nume->Fill(Zboson.Rapidity(),total_weight);
      }
