...
class ZmmAna
{
 public:
  ...
  //function
  ZmmAna(const char *FileList, const char *TypeName,const char *BagNo,const char *lumix,string str);
  virtual ~ZmmAna();
  void Initial(const char *RootName,int RootNumber);
  void Loop(const char *TypeName);
  void End(int RootNumber);
  void Finish(int RootNumber);
  void Output();
...
inline ZmmAna::ZmmAna(const char *FileList,const char *TypeName,const char *BagNo,const char *lumix,string str)
{//func_ZmmAna
 ...
 myHist = new MakeHist();
 TString histName;
 histName = "result/" + (TString)TypeName + "_" + (TString)BagNo + "_" + (TString)lumix + "_result.root";  
 if(isEWMC==true)
 {
  histName = "result_EW/" + (TString)BagNo + "_" + (TString)lumix + "_result.root";
 }
 if(Is_SS == true)
  histName = "./QCD_num/" + (TString)TypeName + "_" +(TString)BagNo + "_" + (TString)lumix + "_result.root";
 
 myHist->BookHist(histName);

 string lumi_new = ".x ../include/Lum_reweight.C";
 if(isEWMC == true)
  lumi_new = ".x /lustre/AtlUser/xwou/ZmmAna/Zmm_bkg/LuminosityReweight/new/" + (TString)BagNo + "_MC_LumiReweight.C";

 string test(BagNo);
 test = test.substr(0,4);
 if(test=="W0lp")lumi_new = ".x /lustre/AtlUser/xwou/ZmmAna/Zmm_bkg/LuminosityReweight/new/W0lp_MC_LumiReweight.C";
 if(test=="W1lp")lumi_new = ".x /lustre/AtlUser/xwou/ZmmAna/Zmm_bkg/LuminosityReweight/new/W1lp_MC_LumiReweight.C";

 gROOT->ProcessLine(lumi_new.c_str());
 string PlotName = "Lum_Scalefactor";
 if(isEWMC == true)
  PlotName="LumiReweight";
 _Lumi_new_file = (TH1D *)gROOT->FindObject((TString)PlotName);

 //2D zpt reweighting
 string temp2 = ".x ../include/Resbos_Pythia_ZPt_Y_Ratio.C";
